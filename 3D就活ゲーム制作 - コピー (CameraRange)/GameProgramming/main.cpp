...
//	gluOrtho2D(-width / 2, width / 2, -height / 2, height / 2);	//2Dの画面を設定
	gluPerspective(75.0, (double)width / (double)height, 1.0, 200000.0);	//3Dの画面を設定
...
int main(void)
{
	...
	window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "3D-RACE", NULL, NULL);
	if (!window)
	{
		glfwTerminate();
		return -1;
...
//		std::cerr << "Can't initialize GLEW" << std::endl;
	...
	glEnable(GL_LIGHT0);
	//ここで光源の位置を決める,4番目0.0にすると平行な光？？
	float lightPosition[] = {0.0f, 100.0f, 100.0f, 0.0f};
